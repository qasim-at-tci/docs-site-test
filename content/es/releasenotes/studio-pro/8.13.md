...
* We reverted a change to the way SVG images are rendered by the [Image](/refguide8/image) and [Image Viewer](/appstore/widgets/image-viewer) widgets. They will be rendered as a `<img src="base64 svg" />` as in previous Mendix versions.
...
* We now render SVG images as actual SVG elements within the [Image](/refguide8/image) and [Image Viewer](/appstore/widgets/image-viewer) widgets for web in order to give you the option to color the SVG. Because the DOM structure is slightly different now for SVG images, styling may be affected. The old situation was `<img src="base64 svg" />`, and the new situation is `<div><svg /></div>` (where the `div` has a relative position).
