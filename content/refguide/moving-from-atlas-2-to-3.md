...
Before you start the upgrade process, it may help if you consult the folder structure changes introduced in Atlas 3 by reading the **File and Folder Structure** section of *How to Customize Styling*. 
...
1.  Download the Atlas 3 ****theme.zip**** and extract it into the root of your Mendix app folder. The folder structure should look similar to the example below. **Mendix app root**, then **theme**, then **web** and **native**:
...
* ****Atlas Core**** - Contains Atlas core styling and layouts
* ****Atlas Web Content**** - Contains Atlas's web page templates and building blocks
* ****Atlas Native Content**** - Contains Atlas's native page templates and building blocks
...
1.  Download ****Atlas Core**** from the Marketplace and replace the existing ***Atlas_UI_Resources*** renamed to ***Atlas_Core***:

	![](attachments/atlas-mig/3-import.png)

#### 2.2.2 Adding Atlas Web Content to Your App

1.  Download ****Atlas Web Content**** from Marketplace
...
1.  Download ****Atlas Native Content**** from Marketplace:
...
Design property options can also be merged across themesource modules. For more information see the **Extending or Overriding Design Properties of Other Modules** section of the **Design Properties API Documentation**.
...
* For errors about the **Phone** or **Tablet** navigation profile no longer existing, right-click the error and select **Go to** which will navigate you to the widget that points to a missing Phone or Tablet profile — use one of these methods to solve the error:
	...
	* In the widget's properties pane change the **Profile** to an already existed profile, like **Responsive web**
	...
	Note that navigation profiles have changed in Mendix 9. See the **Mendix 9 Release Notes** for more information.
...
* If you have errors saying **Unknown option X for design property**, it means the design property option has been removed in Atlas 3. Use one of the following methods to solve the error: 
	...
	* Search for the design property option's CSS class in *theme_atlas2/settings.json* for web and *theme_atlas2/settings-native.json* for native, then add it to the applicable **widget's style property**  
...
* [Atlas 3 Change Summary](atlas3-change-summary)
